...
class Crossword(object):
    def __init__(self, cols, rows, empty = '_', maxloops = 2000, available_words=[]):
        self.cols = cols
        self.rows = rows
        self.empty = empty
    ...
    def display(self, order=True): # return (and order/number wordlist) the grid minus the words adding the numbers
        ...
        for word in self.current_word_list:
            print "{ x : %s, y : %s, num : % s }," % (word.col - 1, word.row - 1, word.number)
    ...
    def legend(self): # must order first
        ...
        for word in self.current_word_list:
            if word.down_across() == "across":
                across.append ({ "no" : word.number, "text" : word.clue})
            else:
                down.append ({ "no" : word.number, "text" : word.clue})
